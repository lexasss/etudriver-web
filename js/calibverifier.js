...
(function (root) {
    ...
    function CalibrationVerifier(customSettings) {
        ...
        var onFullScreenChanged = function (e) {
            root.Utils.debug('CalibrationVerifier', 'onFullScreenChanged, e = ');
            console.log(e);
        ...
        var onFullScreenError = function (e) {
            root.Utils.debug('CalibrationVerifier', 'onFullScreenError, e = ');
            console.log(e);
        };
        ...
        this.run = function (customSettings, customCallbacks) {
            ...
            screenSize = root.Utils.getScreenSize();
        ...
        var fillLocations = function () {
            ...
            for (i = 0; i < 3*testPoints.length; i++) {
                var idx1 = root.Utils.getRandomInt(testPoints.length - 1);
                var idx2 = root.Utils.getRandomInt(testPoints.length - 1);
        ...
        var start = function () {
            root.Utils.debug('CalibrationVerifier', 'starting the verification');
            currentMoveStep = 0;
        ...
        var next = function () {
            ...
            if (currentPointIndex >= 0 && callbacks.targetFinished) {
                root.Utils.debug('CalibrationVerifier', 'collected ' + moveTo.samples.length + ' samples');
                var nextTarget = currentPointIndex + 1 === testPoints.length ? null : testPoints[currentPointIndex + 1];
                if (nextTarget) {
                    nextTarget = {
                        location: root.Utils.clone(nextTarget.location),
                        cell: root.Utils.clone(nextTarget.cell)
                    }
                };
            ...
            if (currentPointIndex === 0 && callbacks.started) {
                ...
                var nextTarget = {
                    location: root.Utils.clone(firstTarget.location),
                    cell: root.Utils.clone(firstTarget.cell)
                };
                callbacks.started(nextTarget);
            }

            if (currentPointIndex < testPoints.length) {
                root.Utils.debug('CalibrationVerifier', 'target #' + currentPointIndex);
                if (settings.transitionDuration > 0) {
                    root.Utils.debug('CalibrationVerifier', 'animating');
        ...
        var show = function () {
            root.Utils.debug('CalibrationVerifier', 'target is displaying');
            ...
            if (callbacks.targetStarted) {
                var currentTarget = {
                    location: root.Utils.clone(testPoint.location),
                    cell: root.Utils.clone(testPoint.cell)
                }
        ...
        var reportResult = function () {
            root.Utils.debug('CalibrationVerifier', 'reporting results');
            var i, len;
